...
function Visual.SetupPlayerAdvancedESP(plr)
    ...
    local charRemovingConnection = plr.CharacterRemoving:Connect(handleCharacterChanged)
...
function Visual.CacheAdvancedESPCharacter(d, char)
    if d.Character == char and d.Root and d.Root.Parent and d.Head and d.Head.Parent and d.Humanoid and d.Humanoid.Parent then
        return
    end

    d.Character = char
    d.Root = char:FindFirstChild("HumanoidRootPart")
    d.Head = char:FindFirstChild("Head")
    ...
    if Visual.IsR6(char) then
        ...
        local leftLowerLeg = char:FindFirstChild("LeftLowerLeg") or char:FindFirstChild("Left Shin")
        local leftFoot = char:FindFirstChild("LeftFoot") or char:FindFirstChild("Left foot")
        local rightUpperLeg = char:FindFirstChild("RightUpperLeg") or char:FindFirstChild("Right Leg")
        local rightLowerLeg = char:FindFirstChild("RightLowerLeg") or char:FindFirstChild("Right Shin")
        local rightFoot = char:FindFirstChild("RightFoot") or char:FindFirstChild("Right foot")
...
function Visual.UpdateAdvancedESP()
    ...
    for plr, d in pairs(Visual.AdvancedESP.espObjects) do
        ...
        if char then
            Visual.CacheAdvancedESPCharacter(d, char)
        end
            ...
            if onScreen then
                ...
                if d.HealthBg and d.HealthBar and d.HealthText then
                    ...
                    if settings.healthbar and settings.enabled then
                        local HEALTH_STRIPES = settings.healthStripes or 24
                ...
                if d.Bones then
                    ...
                    local bones = d.BonePairs or {}

                    for i = 1, 14 do
                        local line = d.Bones[i]
                        if bonesVisible and line and bones[i] and bones[i][1] and bones[i][2] then
